...
def _resolve_locator(w: "_BrowserWorker", ref=None, selector=None, text=None, role=None, name=None):
    ...
    # 1. ref (eN) — Stonic-style precise targeting with automatic self-healing
    if ref:
        old_info = w.element_map.get(ref)
        
        # Try resolving using standard map first
        if old_info:
            try:
                return _resolve_locator_with_strategies(w, old_info["role"], old_info.get("name"), old_info.get("nth"))
            except Exception:
                pass
                
        # Self-Healing: Re-scan page to find updated element or handle stale references
        import logging
        logger = logging.getLogger("myraa.browser")
        logger.info("Self-healing: Element reference '%s' resolution failed. Rebuilding element map...", ref)
        ...
        if new_ref:
            ...
            info = w.element_map[new_ref]
            return _resolve_locator_with_strategies(w, info["role"], info.get("name"), info.get("nth"))
        elif old_info:
            # Last-ditch effort: try old info on the fresh DOM
            return _resolve_locator_with_strategies(w, old_info["role"], old_info.get("name"), old_info.get("nth"))
        else:
...
def _safe_click_loc(loc, timeout=5000) -> str:
    ...
    # 3. Try JS-based click (direct element click bypasses overlays/layout shifts)
    try:
