...
def _find_tesseract_exe() -> Optional[str]:
    ...
    Search order:
      ...
      2. User-specified custom path (D:\\APP\\New Trasarect)
      3. Standard installation paths
    Also sets TESSDATA_PREFIX if tessdata directory is found alongside.
    """
    candidates = [
        r"D:\APP\New Trasarect\tesseract.exe",
        r"D:\APP\New Trasarect\tesseract",
        r"C:\Program Files\Tesseract-OCR\tesseract.exe",
        r"C:\Program Files (x86)\Tesseract-OCR\tesseract.exe",
        r"C:\Users\mdnir\AppData\Local\Programs\Tesseract-OCR\tesseract.exe",
    ]
    for c in candidates:
        if os.path.isfile(c):
            parent_dir = os.path.dirname(c)
            # Prepend to PATH so DLLs (libtesseract-5.dll, etc.) resolve
            if parent_dir and parent_dir not in os.environ.get("PATH", ""):
                os.environ["PATH"] = parent_dir + os.pathsep + os.environ.get("PATH", "")
            # Set TESSDATA_PREFIX so tessdata/ dir is found for lang data
