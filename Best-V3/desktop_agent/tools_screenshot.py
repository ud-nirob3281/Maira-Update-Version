...
def capture_screen_unified() -> "Any":
    """Unified screen capture using PIL ImageGrab with all_screens=True and cursor drawing."""
    try:
        ...
        try:
            ...
            # Map physical cursor coordinates to image coordinates
            ix = cx - vx
            iy = cy - vy
...
def take_screenshot(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not force and SNAPSHOT_CACHE.check_and_update_state(worker=None):
        if SNAPSHOT_CACHE.cached_screenshot is not None:
            return SNAPSHOT_CACHE.cached_screenshot
    ...
    # Cache this screenshot payload
    SNAPSHOT_CACHE.populate_cache(worker=None, current_img=img, screenshot_data=result)
    return result
...
def analyze_screenshot(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    force = bool(args.get("force", False))
    if not force and SNAPSHOT_CACHE.check_and_update_state(worker=None):
        if SNAPSHOT_CACHE.cached_ocr_result is not None:
            return SNAPSHOT_CACHE.cached_ocr_result
    ...
    # Cache OCR result
    SNAPSHOT_CACHE.populate_cache(worker=None, current_img=img, ocr_result=result)
    return result
...
def read_screen(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    force = bool(args.get("force", False))
    if not force and SNAPSHOT_CACHE.check_and_update_state(worker=None):
        if SNAPSHOT_CACHE.cached_read_screen is not None:
            return SNAPSHOT_CACHE.cached_read_screen
    ...
    # Cache readScreen result
    SNAPSHOT_CACHE.populate_cache(worker=None, current_img=img, read_screen_data=result)
    return result
